...
**Your app owns its scroller — the window cannot scroll it for you.** The window's content area is `overflow: auto`, which scrolls a *builtin* app just fine. Your app is an iframe sized to `height: 100%` of that box: never taller than the box, so the shell's scrollbar never appears. Put `overflow: hidden` and a fixed `height` on your root with no `overflow: auto` under it and the bottom of every long view is unreachable — no scrollbar, no keyboard scroll, nothing. It reads fine in a full-screen tab with three seeded rows and fails in a 1280×860 window with twelve, which is why it has shipped three times. **Before you deploy, open the app at 900×600 with roughly three times your seeded data and check that the bottom of every view is reachable.** The rule and the fix are in `references/design.md` → "Window rules"; since `manaurum.js` 1.12.0 / `manaurum-v2.mjs` 2.3.0 the SDK also console-errors when it catches this, and names the element.
