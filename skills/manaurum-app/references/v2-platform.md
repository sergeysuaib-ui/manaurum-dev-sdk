...
| `permissions` | string[] | BROWSER features the OS shell delegates to the app iframe via the `allow` attribute (Permissions-Policy). Enum today: `microphone` (MAN-1316) and `camera` (MAN-1889, landed by MAN-1920). Required for a LIVE `getUserMedia` stream inside the shell iframe; a still photo through `<input type="file" capture="environment">` is not gated and needs no declaration. The user still sees the browser's own prompt. `uniqueItems` — a repeated value is a 422. Refused when `runtime.mode` is `byo`, and the shell delegates nothing to any app-named origin (MAN-1922). Unrelated to `requires_capabilities` — a voice app needs both this AND `os.ai.transcribe`. |
